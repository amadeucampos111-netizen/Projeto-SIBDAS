...
// ==========================================
// PASSO 1: PROCURAR INFORMAÇÃO DA GARANTIA NO ARQUIVO INATIVO
// ==========================================
$sql_busca = "SELECT gc.id, e.designacao, e.numero_serie, gc.data_inicio_garantia, gc.data_fim_garantia 
              ...
              WHERE gc.id = ? AND gc.estado = 'Inativo'";
...
// Se o registo não for encontrado ou já não estiver Inativo, regressa à lista de inativos
if (!$contrato) {
    $_SESSION['msg_erro'] = "O registo de garantia não foi encontrado no arquivo ou já se encontra ativo.";
    header("Location: ../listar/lista_garantias_inativas.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirmar_reativacao'])) {
    ...
    // UPDATE para restaurar a cobertura alterando o estado de volta para 'Ativo'
    $sql_update = "UPDATE garantias_contratos SET estado = 'Ativo' WHERE id = ?";
    ...
    if ($stmt_update) {
        ...
        if (mysqli_stmt_execute($stmt_update)) {
            $_SESSION['msg_sucesso'] = "A cobertura de garantia do equipamento '" . $contrato['designacao'] . "' foi reativada com sucesso!";
            mysqli_close($conn);
            // Redireciona para a lista principal de contratos ativos
            header("Location: ../listar/lista_garantias.php");
            exit;
        } else {
            $_SESSION['msg_erro'] = "Erro técnico ao tentar restaurar o registo: " . mysqli_stmt_error($stmt_update);
        }
        mysqli_stmt_close($stmt_update);
    } else {
        $_SESSION['msg_erro'] = "Erro interno ao preparar a reativação.";
    }
...
<head>
    <meta charset="UTF-8">
    <title>MedTrack | Confirmar Reativação de Garantia</title>
        ...
        <div class="col-md-6">
            ...
            <div class="card shadow-sm border-0 rounded-3 text-center p-4">
                <div class="card-body">
                    ...
                    <h4 class="fw-bold text-dark mb-3">Reativar Registo de Garantia?</h4>
                    
                    <p class="text-muted mb-4">
                        Tem a certeza que deseja restaurar esta cobertura contratual? O registo voltará a figurar nas listagens diárias e os alertas de validade associados ao equipamento voltarão a ser processados.
                    </p>
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="POST">
                        ...
                        <div class="d-flex justify-content-center gap-3">
                            <a href="../listar/lista_garantias_inativas.php" class="btn btn-light px-4 border">
